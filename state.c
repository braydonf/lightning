			  ...
			  const struct bitcoin_tx **broadcast)
{
	...
	*broadcast = NULL;
	...
	case STATE_NORMAL:
		 ...
		 *
		 * There's nothing wrong with doing this, we just need to
		 * track what was commited to before we sent commit, so we can
		 * stage only those changes on our commit when we get
		 * complete message.
		 */

		/* You can only issue this command one at a time. */
		if (input_is(input, CMD_SEND_COMMIT)) {
			queue_pkt(out, pkt_commit(ctx, peer));
			change_peer_cond(peer, PEER_CMD_OK, PEER_BUSY);
		...
		} else if (input_is(input, CMD_SEND_HTLC_UNADD)) {
			/* We are to send an HTLC un-add. */
			queue_pkt(out,
				  pkt_htlc_unadd(ctx, peer, idata->htlc_prog));
			return instant_cmd_success(peer, cstatus);
		} else if (input_is(input, CMD_SEND_HTLC_FULFILL)) {
			/* We are to send an HTLC fulfill. */
			queue_pkt(out,
				  pkt_htlc_fulfill(ctx, peer, idata->htlc_prog));
			return instant_cmd_success(peer, cstatus);
	...
	case STATE_NORMAL_COMMITTING:
		/* Only expect complete in STATE_NORMAL_COMMITTING */
		if (peer->state == STATE_NORMAL_COMMITTING
		    && input_is(input, PKT_UPDATE_COMPLETE)) {
			err = accept_pkt_complete(ctx, peer, idata->pkt);
			if (err) {
				complete_cmd(peer, &cstatus, CMD_FAIL);
				goto err_start_unilateral_close;
			}
		...
		if (input_is(input, CMD_CLOSE)) {
			...
			err = accept_pkt_htlc_add(ctx, peer, idata->pkt);
			if (err)
				goto err_start_unilateral_close;
			return unchanged_state(cstatus);
		} else if (input_is(input, PKT_UPDATE_UNADD_HTLC)) {
			err = accept_pkt_htlc_unadd(ctx, peer, idata->pkt);
			if (err)
				goto err_start_unilateral_close;
			return unchanged_state(cstatus);
			...
			err = accept_pkt_commit(ctx, peer, idata->pkt);
			if (err)
				goto err_start_unilateral_close;
			queue_pkt(out, pkt_complete(ctx, peer));
			return unchanged_state(cstatus);
		} else if (input_is(input, BITCOIN_ANCHOR_THEIRSPEND)) {
			goto them_unilateral;
		} else if (input_is(input, BITCOIN_ANCHOR_OTHERSPEND)) {
